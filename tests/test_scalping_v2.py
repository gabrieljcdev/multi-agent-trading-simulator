...
from agents.scalping_confluence import ConfluenceChecker, ConfluenceResult, CombinedConfluenceResult
from agents.scalping_atr_sl import ATRStopCalculator, TpSlV2
from agents.scalping_agent_v2_integration import is_ready_for_live_v2


def _v2_settings():
    """Mutable snapshot of the integrated SCALP_* settings, so tests can flip
    individual flags without mutating the real config.settings module."""
    s = types.SimpleNamespace()
    for name in dir(_cfg):
        if name.startswith("SCALP_"):
            setattr(s, name, getattr(_cfg, name))
    return s
